...
class JadwalUpahScreen extends StatefulWidget {
  const JadwalUpahScreen({super.key});

  @override
  State<JadwalUpahScreen> createState() => _JadwalUpahScreenState();
}

class _JadwalUpahScreenState extends State<JadwalUpahScreen> {
                          ...
                          onTap: () {
                            ...
                            if (title.isEmpty || worker.isEmpty) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                    content:
                                    Text('Judul & pekerja wajib diisi')),
                              );
                              return;
                            }
                            ...
                            setState(() {
                              final list =
                              List<_UpahSchedule>.from(_activeSchedules);
                              list.insert(0, newItem);
                              schedulesByDay[selectedDay] = list;

                              dots[selectedDay] =
                              (dots[selectedDay] ?? [])..add(const Color(0xFF735BF2));
                            });
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                  content:
                                  Text('Jadwal upah ditambahkan (dummy)')),
                            );
                          },
                          child: Container(
                            height: 44,
                  ...
                  Row(
                    children: [
                      const Icon(Icons.location_on_outlined,
                          size: 18, color: Color(0xFF575A66)),
