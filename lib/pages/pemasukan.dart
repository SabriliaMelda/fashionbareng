...
class _PemasukanScreenState extends State<PemasukanScreen> {
  _InTab _tab = _InTab.input;

  // filter periode (dummy, biar konsisten gaya)
  _Period _period = _Period.bulanan;
  DateTimeRange? _customRange;

  // pilihan proyek (dummy)
  _Project _project = _Project.a;

  final List<_IncomeTxn> _items = [
    _IncomeTxn(
      date: DateTime.now().subtract(const Duration(days: 1)),
      title: 'DP pelanggan',
      note: 'Order seragam 50pcs',
      amount: 1250000,
    ),
    _IncomeTxn(
      date: DateTime.now().subtract(const Duration(days: 3)),
      title: 'Pelunasan',
      note: 'Order kaos komunitas',
      amount: 2300000,
    ),
    _IncomeTxn(
  ...
  int get _totalIncome => _items.fold(0, (a, b) => a + b.amount);
          ...
          child: Column(
            ...
            children: [
              ...
              _InputField(
                controller: titleC,
                label: 'Sumber',
                hint: 'Contoh: DP / Pelunasan / Penjualan',
              ),
              const SizedBox(height: 10),
              _InputField(
                controller: noteC,
                label: 'Catatan',
                hint: 'Contoh: Order seragam 50pcs',
              ),
              const SizedBox(height: 10),
              _InputField(
                controller: amountC,
                  ...
                  Expanded(
                    child: InkWell(
                      ...
                      onTap: () {
                        ...
                        final amt = int.tryParse(amountC.text.replaceAll('.', '')) ?? 0;

                        if (title.isEmpty || amt <= 0) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Sumber & nominal wajib diisi')),
                          );
                          return;
                        }
        ...
        children: [
          Expanded(
            child: _TabPill(
              ...
              active: _tab == _InTab.input,
              onTap: () => setState(() => _tab = _InTab.input),
            ),
          ),
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 10),
                      const Icon(Icons.more_vert_rounded, color: Color(0xFF9AA4B2), size: 18),
                    ],
                  ),
                ),
              );
...
enum _InTab { input, edit, project }
enum _Project { a, b }
