...
class _ListrikScreenState extends State<ListrikScreen> {
  ...
  List<_MonthlyPay> _buildLast12() {
    ...
    final amounts = <int>[
      910000, 945000, 980000, 875000, 920000, 890000,
      930000, 960000, 905000, 940000, 970000, 975000,
    ];
    for (int i = 0; i < 12; i++) {
  ...
  int get _max12 => _last12.isEmpty ? 0 : _last12.map((e) => e.amount).reduce((a, b) => a > b ? a : b);
  int get _min12 => _last12.isEmpty ? 0 : _last12.map((e) => e.amount).reduce((a, b) => a < b ? a : b);
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            _Chip(
                              text: _currentBill.status == _BillStatus.lunas ? 'Lunas' : 'Belum bayar',
                        ...
                        Row(
                          children: [
                            Expanded(
                              child: _MiniStat(label: 'kWh', value: '${_currentBill.kwh}'),
                            ),
                            const SizedBox(width: 10),
                            Expanded(
                              child: _MiniStat(label: 'Rata-rata 12 bln', value: _rupiah(_avg12)),
                            ),
                          ],
                        ),
                        const SizedBox(height: 12),
                          ...
                          children: [
                              ...
                              child: _GhostButton(
                                ...
                                onTap: () {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('Export PDF listrik (dummy)')),
                                  );
                                },
                              ),
                            ),
                            ...
                            Expanded(
                              child: _GhostButton(
                                ...
                                onTap: () {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('Bayar listrik (dummy)')),
                                  );
                                },
                              ),
                            ),
                  ...
                  _InfoCard(
                    ...
                    child: Column(
                      children: [
                        Row(
                          children: [
                            Expanded(child: _MiniStat(label: 'Tertinggi', value: _rupiah(_max12))),
                            const SizedBox(width: 10),
                            Expanded(child: _MiniStat(label: 'Terendah', value: _rupiah(_min12))),
                          ],
                        ),
                        const SizedBox(height: 12),
                        ..._last12.map((m) {
